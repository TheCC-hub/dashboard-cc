...
const platforms = [
    {
        name: 'WhatsApp',
        icon: <FaWhatsapp className="text-green-500 text-xl" size={40} />,
        link: 'https://wa.me/yourNumber',
        description: 'Chat with us instantly on WhatsApp.',
    },
    {
        name: 'Discord',
        icon: <FaDiscord className="text-indigo-500 text-xl" size={40} />,
        link: 'https://discord.gg/yourInviteCode',
        description: 'Join our Discord server for real-time support.',
    },
    {
        name: 'Slack',
        icon: <FaSlack className="text-purple-500 text-xl" size={40} />,
        link: 'https://yourworkspace.slack.com',
        description: 'Connect with us on Slack for async collaboration.',
    },
    {
        name: 'Email',
        icon: <FaEnvelope className="text-red-500 text-xl" size={40} />,
        link: 'mailto:hello@example.com',
        description: 'Send us an email anytime.',
    },
    {
        name: 'Telegram',
        icon: <FaTelegramPlane className="text-blue-400 text-xl" size={40} />,
        link: 'https://t.me/yourHandle',
        description: 'Reach us via Telegram for quick queries.',
    },
    {
        name: 'Messenger',
        icon: <FaFacebookMessenger className="text-blue-600 text-xl" size={40} />,
            ...
            <div className="grid grid-cols-3 justify-center gap-4 mb-4 overflow-x-auto">
                {platforms.map((platform, idx) => (
                    <Link
                        ...
                        href={platform.link}
                        target='_blank'
                        className={`flex flex-col items-center gap-5 px-3 py-10 text-lg rounded-xl transition hover:bg-red-500 bg-zinc-800`}
                    >
                        {platform.icon}
                        {platform.name}
                    </Link>
